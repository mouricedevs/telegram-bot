...
module.exports = {
    ...
    onStart: async function ({ bot, chatId, msg }) {
        ...
        if (!msg.reply_to_message || !(msg.reply_to_message.audio || msg.reply_to_message.video || msg.reply_to_message.photo)) {
            return bot.sendMessage(chatId, "Please reply to an audio or video  or photo message to send its file link.");
        }

        try {
            
            const fileId = msg.reply_to_message.audio ? msg.reply_to_message.audio.file_id : msg.reply_to_message.photo[0].file_id ||               msg.reply_to_message.video.file_id ;
            ...
            bot.sendMessage(chatId, `File Link: ${fileLink}`);
        } catch (error) {
